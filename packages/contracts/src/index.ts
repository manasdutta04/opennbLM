...
export interface RumikApi { getStatus(): Promise<RumikStatus>; start(): Promise<void>; stop(): Promise<void>; healthCheck(): Promise<boolean>; synthesize(text: string, config?: Partial<RumikConfig>): Promise<{ segments: Array<{ id: string; text: string; wavPath: string }> }>; cancel(): Promise<void>; getVoices(): Promise<readonly string[]>; }
export interface PreloadApi { getAppInfo(): Promise<AppInfo>; conversations: ConversationApi; providers: ProviderSettingsApi; rumik: RumikApi; }

...
export function createRumikManager(options: RumikManagerOptions): RumikManager {
  ...
  const emitState = () => { const next = { ...status }; stateListeners.forEach((listener) => listener(next)); };
  ...
  return { detectRuntime, detectModel, getStatus: () => ({ ...status }), onSegmentReady(listener) { segmentListeners.add(listener); return () => segmentListeners.delete(listener); }, onStateChange(listener) { stateListeners.add(listener); return () => stateListeners.delete(listener); }, async start() { status.state = "preparing"; emitState(); const runtime = await detectRuntime(); if (!runtime) { status = { ...status, state: "error", error: `Python runtime not found: ${python}` }; emitState(); throw new Error(status.error); } await detectModel(); status.state = "idle"; emitState(); }, async stop() { await this.cancel(); status.state = "idle"; emitState(); }, async healthCheck() { return (await detectRuntime()) && (await detectModel()); }, async synthesize(text, override = {}) { if (!text.trim()) return { segments: [] }; if (!status.runtimeAvailable) await this.start(); if (!status.modelAvailable) { status = { ...status, state: "error", error: "Rumik model is not available locally" }; emitState(); throw new Error(status.error); } status.state = "preparing"; emitState(); const config = { ...defaultConfig, ...override }; const segments = segmentForRumik(text); const results: RumikAudioSegment[] = []; for (let index = 0; index < segments.length; index += 1) { const segment = segments[index]; const wavPath = join(options.outputDirectory, `rumik-${Date.now()}-${index}.wav`); await new Promise<void>((resolvePromise, reject) => { currentProcess = spawn(python, [runner, "--model-path", options.modelPath!, "--revision", RUMIK_MODEL_REVISION, "--text", segment, "--speaker", config.speaker, "--temperature", String(config.temperature), "--top-k", String(config.topK), "--max-new-tokens", String(config.maxTokens), "--description", config.deliveryDescription, "--language", config.language, "--output", wavPath], { stdio: ["ignore", "ignore", "pipe"] }); let error = ""; currentProcess.stderr?.on("data", (data) => { error += String(data); }); currentProcess.once("error", reject); currentProcess.once("exit", (code) => { currentProcess = undefined; if (code === 0) resolvePromise(); else reject(new Error(error || `Rumik exited with code ${code}`)); }); }); const ready = { id: `${Date.now()}-${index}`, text: segment, wavPath }; results.push(ready); status.state = "speaking"; emitState(); segmentListeners.forEach((listener) => listener(ready)); } return { segments: results }; }, async cancel() { if (currentProcess) { currentProcess.kill(); currentProcess = undefined; } status.state = "paused"; emitState(); }, getVoices: () => RUMIK_SPEAKERS };
}
